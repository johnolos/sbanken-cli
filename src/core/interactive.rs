...
pub fn fuzzy_match_account<'a>(accounts: &'a [AccountObj], header: &str) -> Result<&'a AccountObj, Error> {
    ...
    for account in accounts {
        input.push_str(&format!("{}\t\t[nr: {}]\n", account.name, account.account_number))
    }

    io::copy(&mut input.as_bytes(), fzf.stdin.as_mut().unwrap()).expect("Could not pipe stdin to fzf process");
...
pub fn remove_account<'a>(accounts: &'a mut Vec<AccountObj>, account_id: &str) -> &'a mut Vec<AccountObj> {
